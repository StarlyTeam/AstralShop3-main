    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (args.length == 0) {
            ...
            for (SubCommand subCommand : uniqueSubCommands) {
                if (isKor(label)) {
                    sender.sendMessage(" §e§l| §r§f/상점관리 " + subCommand.getKorName() + " " + subCommand.getKorUsage() + " §7" + subCommand.getKorDescription());
                } else {
                    sender.sendMessage(" §e§l| §r§f/shopadmin " + subCommand.getEngName() + " " + subCommand.getKorUsage() + " §7" + subCommand.getKorDescription());
                }
            }
            sender.sendMessage("");
            return true;
